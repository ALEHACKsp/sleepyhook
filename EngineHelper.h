...
bool EngineHelper::Internal::WorldToScreen(Vector3 vWorldPos, Vector3* vScreenPos)
{
    ...
    if (!pW2SAddress)
    {
        pW2SAddress = MemoryHelper::Pattern::PatternScan(_xor("48 89 5C 24 ? 48 89 6C 24 ? E9 ? ? ? ? E9 4F C6 41 08").c_str());  // dont know
        if (!pW2SAddress)
        {
            Settings::bNeedUnHook = true;
...
void EngineHelper::UObjects::FreeObjName(uintptr_t pAddress)
{
    ...
    {
        pFreeObjNameAddress = MemoryHelper::Pattern::PatternScan(_xor("48 85 C9 74 2E 53 48 83 EC 20 48 8B D9 48 8B 0D ? ? ? ? 48 85 C9 75 0C").c_str());
        if (!pFreeObjNameAddress)
        {
...
char* EngineHelper::UObjects::GetObjectName(uintptr_t pObject, bool bSkipFix)
{
    ...
    if (!pGetObjectNameAddress)
    {
        ...
        if (!pGetObjectNameAddress)  //updated 
        {
            Settings::bNeedUnHook = true;
...
char* EngineHelper::UObjects::GetNameByIndex(int32_t nIndex, bool bSkipFix)
{
    ...
    {
        pGetNameByIndexAddress = MemoryHelper::Pattern::PatternScan(_xor("E8 ? ? ? ? 39 5D 38").c_str()); //   48 89 5C 24 ? 55 56 57 48 8B EC 48 83 EC 30 8B
        if (!pGetNameByIndexAddress)
        {
            Settings::bNeedUnHook = true;
