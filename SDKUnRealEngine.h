...
public:
	union
	{
		DEFINE_MEMBER_N(Vector3, RootComponent, 0x130);
		DEFINE_MEMBER_N(Vector3, RelativeLocation, 0x11C);
		DEFINE_MEMBER_N(Vector3, RelativeRotation, 0x0128);
		DEFINE_MEMBER_N(Vector3, ComponentVelocity, 0x140);
	};
};
...
public:
	union
	{
		DEFINE_MEMBER_N(int32_t, TeamIndex, 0xE68);
	};
};
class cStaticMesh
{
...
public:
	union
	{
		DEFINE_MEMBER_N(BYTE, Tier, 0x18);
		DEFINE_MEMBER_N(FText*, DisplayName, 0x70);
	};
};
...
public:
	union
	{
		DEFINE_MEMBER_N(FLOAT, CustomTimeDilation, 0x98);
		DEFINE_MEMBER_N(cSceneComponent*, SceneComponent, 0x0130);
		DEFINE_MEMBER_N(cFortPlayerStateAthena*, PlayerState, 0x0240);

		DEFINE_MEMBER_N(uintptr_t*, PlayerMesh, 0x0280);
		DEFINE_MEMBER_N(BYTE, bIsDying, 0x538);
		DEFINE_MEMBER_N(BYTE, bIsDBNO, 0x552);
		DEFINE_MEMBER_N(uintptr_t, CurrentWeapon, 0x5A0);
		DEFINE_MEMBER_N(uintptr_t, WeaponData, 0x378);
		//FortProjectileAthena
		DEFINE_MEMBER_N(Vector3, FireLoc, 0x0868);

		//BuildingContainer
		DEFINE_MEMBER_N(BYTE, bAlreadySearched, 0xC71);

		//FortPickup/PrimaryPickupItemEntry + FortItemEntry/ItemDefinition
		DEFINE_MEMBER_N(cFortItemDefinition*, ItemDefinition, 0x298 + 0x0018);

		//FortHoagieVehicle
		DEFINE_MEMBER_N(cFortHoagieVehicleConfigs*, HoagieVehicleConfigs, 0x0FB8);

		//FortMeatballVehicle
		DEFINE_MEMBER_N(cFortMeatballVehicleConfigs*, MeatballVehicleConfigs, 0x1188);
	};
...
public:
	union
	{
		DEFINE_MEMBER_N(cActor*, AcknowledgedPawn, 0x2A0);
...
public:
	union
	{
		DEFINE_MEMBER_N(cLocalPlayer**, LocalPlayers, 0x38);
	};
};
